...
def store_split(datasets: list|ConcatDataset, containers: list|str) -> None:
    """
    Store a new dataset split, creating new files or updating existing ones.
    ...
    datasets : list|ConcatDataset
        Data splits to save.
    containers : list|str
        File paths (files to be created or to be updated).
    
    Returns
    -------
    None
    """
    if type(datasets) is ConcatDataset:
        datasets = [datasets]
    if type(containers) is str:
        containers = [containers]
    
    for dataset, container in zip(datasets, containers):
        if not os.path.exists(container):
            print(f"Creating '{container}' ...")
            concat_ds_datasets = []
        else:
            print(f"Updating '{container}' ...")
            concat_ds = torch.load(os.path.join(container), weights_only=False)
            concat_ds_datasets = concat_ds.datasets
    
        if dataset is None:
            ds = TensorDataset(torch.empty(0))
        else:
            ds = dataset

        head, _ = os.path.split(container)
        os.makedirs(head, exist_ok=True)
        torch.save(ConcatDataset(concat_ds_datasets + [ds]), container)